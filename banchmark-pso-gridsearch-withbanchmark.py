...
def parse_tai_file(filename):
    """Parseur robuste pour les fichiers TAI avec gestion des en-têtes"""
    instances = []
    ...
    i = 0
    while i < len(lines):
        # Détection du début d'une nouvelle instance
...
def grid_search(jobs):
    best = {'params': None, 'makespan': float('inf')}
    
    for config in PSOConfig.generate_configs():
        makespan, _ = pso_jssp(jobs, **config)
...
if __name__ == "__main__":
    ...
    for idx, instance in enumerate(instances, 1):
        ...
        # Grid Search
        best_config = grid_search(instance['jobs'])
        print(f"Meilleurs paramètres: {best_config['params']}")
        
        # Optimisation finale
        makespan, schedule = pso_jssp(instance['jobs'], **best_config['params'])
